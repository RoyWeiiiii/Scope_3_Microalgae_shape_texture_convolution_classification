##################################### Multiple image processing for Hu_moments features (Gray-scaling, Bilateral blur, & Adaptive Thresh) #####################
import cv2
import numpy as np
import os
import pandas as pd
...
if scaler_type == 'minmax':
    export_path = os.path.join(root_dir, 'Input export path to save file name as.csv')
else:
    export_path = os.path.join(root_dir, 'Input export path to save file name as.csv')
